 ...
 */
 ...
 class Waiter extends Staff {
    ...
    async _doneWork(order, cook) {
        ...
        if(order.length !== undefined && typeof order !== "string") {//先去厨师那边；

            let position = this.dom.byCook('order', cook);//去厨师那边传递菜单；
            ...
            let position = this.dom.byCook('pass', cook);//去厨房取菜；
            ...
            this.dom.byCustomer(order.index);//上菜；
 ...
 */
 ...
 class Cook extends Staff {
    ...
    async _doneWork(menu) {//一次一次做菜。同一个厨师，做完之后再做下一道菜。
 ...
 class Customer {
    ...
    async _order(menu) {
        ...
        for(let i = this.baseTime; i >= 0; i--) {
            ...
            this.dom.updateOrderState(i)//倒计时;
