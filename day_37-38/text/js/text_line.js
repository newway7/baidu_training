Line.prototype = {
    ...
    createPath: function (path) {
      ...
      for(let i = 0; i < path.length; i++) {//画点
  ...
  //this.contentHeight - this.offset * 2折线图高度；
    },
    ...
    drow: function (data) {
      ...
      this.createText(0 + this.offset, 10 + this.offset, this.valueMax)//y轴最大值
  
      this.createText(0 + this.offset, this.contentHeight - this.offset, 0)//y轴最小值
    ...
    init: function () {
      ...
      this.lineWidth = (this.contentWidth - this.offset*2 ) / 12;//宽度，这与bar图不同；
  ...
  function Line(node) {
    ...
    this.offset = 15;//折线图与边界距离
  
    this.contentWidth = this.width - this.offset;//折线图宽度+2this.offset
  
    this.contentHeight = this.height - this.offset;//折线图高度-this.offset
